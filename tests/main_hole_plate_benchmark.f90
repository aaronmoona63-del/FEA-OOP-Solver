program main_hole_plate_benchmark
  use Types, only : prec
  ...
  real(prec) :: ux_pardiso, ux_cg, ux_pcg

  print *, "========================================================="
  print *, " 👑 [终局之战] 58.8万自由度 速度与精度客观评测 👑"
  print *, "========================================================="

  call load_mesh_from_txt('/work/tests/mesh_daikonglashen_dense.txt', n_nodes, n_elements, coords, conn, props)
  n_unknowns = n_nodes * 3
  if (props(1) < 1.0_prec) props(1) = 210000.0_prec
  if (props(2) < 0.01_prec) props(2) = 0.3_prec

  print *, ">>> [1/3] 正在装配全局刚度矩阵..."
  ...
  ux_pardiso = get_right_face_ux(n_nodes, coords, x)
  print *, "    ✅ 耗时: ", time_pardiso, " 秒 | 右端位移 Ux: ", ux_pardiso, " mm"
  ...
  ux_cg = get_right_face_ux(n_nodes, coords, x)
  print *, "    ✅ 耗时: ", time_cg, " 秒 | 右端位移 Ux: ", ux_cg, " mm"
  call solver%free(); deallocate(solver)
  ...
  ux_pcg = get_right_face_ux(n_nodes, coords, x)
  print *, "    ✅ 耗时: ", time_pcg, " 秒 | 右端位移 Ux: ", ux_pcg, " mm"
  call solver%free(); deallocate(solver)
...
end program main_hole_plate_benchmark
